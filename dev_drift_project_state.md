...
6. [Phase 1 — Implementation Checklist](#6-phase-1--implementation-checklist)
7. [Phase 2 — Pending Requirements](#7-phase-2--pending-requirements)
...
| **Bundler** | Turbopack | Next.js 16 default (dev & build) |
...
| Trigger | Table | Fires | Function | Purpose |
|---|---|---|---|---|
| `handle_profiles_updated_at` | `profiles` | BEFORE UPDATE | `extensions.moddatetime(updated_at)` | Auto-set `updated_at` |
| `handle_listings_updated_at` | `listings` | BEFORE UPDATE | `extensions.moddatetime(updated_at)` | Auto-set `updated_at` |
| `on_auth_user_created` | `auth.users` | AFTER INSERT | `public.handle_new_user()` | Auto-create profile row on signup |
...
```sql
-- Triggered on auth.users INSERT. Creates a profiles row using metadata from signup.
-- Falls back to 'user_<first-8-of-uuid>' for username if none provided.
CREATE OR REPLACE FUNCTION public.handle_new_user() RETURNS TRIGGER
  LANGUAGE plpgsql SECURITY DEFINER SET search_path = '';
```

#### `match_listings()` — Semantic Vector Search

```sql
-- Returns top-N published listings most similar to a query embedding.
-- Parameters: query_embedding vector(384), match_threshold FLOAT (default 0.75),
--             match_count INT (default 10)
-- Returns: id, title, description, type, location, popularity_score, similarity
...
**Usage example:**
```sql
SELECT * FROM match_listings('[0.1, 0.2, ...]'::vector(384), 0.78, 10);
```

### 5.7 Row Level Security (RLS)

All three tables have RLS **enabled**. Policies follow a **read-public / write-own** pattern:

| Table | Operation | Policy | Target |
|---|---|---|---|
| **profiles** | SELECT | Viewable by everyone | `anon`, `authenticated` |
| **profiles** | UPDATE | Own profile only | `auth.uid() = id` |
| **profiles** | DELETE | Own profile only | `auth.uid() = id` |
| **listings** | SELECT | Published only (`is_published = true`) | `anon`, `authenticated` |
| **listings** | INSERT | Authenticated, own `created_by` | `authenticated` |
| **listings** | UPDATE | Own listings only | `auth.uid() = created_by` |
| **listings** | DELETE | Own listings only | `auth.uid() = created_by` |
| **interactions** | SELECT | Own interactions only | `auth.uid() = user_id` |
| **interactions** | INSERT | Authenticated, own `user_id` | `authenticated` |
| **interactions** | DELETE | Own interactions only (unsave) | `auth.uid() = user_id` |
...
-- profiles:   SELECT (anon + auth), INSERT/UPDATE/DELETE (auth)
-- listings:   SELECT (anon + auth), INSERT/UPDATE/DELETE (auth)
-- interactions: SELECT/INSERT/DELETE (auth only)
-- match_listings(): EXECUTE (anon + auth)
```
...
|---|---|---|
| `username_length` | profiles | `char_length(username) BETWEEN 3 AND 40` |
| `bio_length` | profiles | `char_length(bio) <= 500` |
| `title_length` | listings | `char_length(title) BETWEEN 1 AND 200` |
| `popularity_non_negative` | listings | `popularity_score >= 0` |
| `valid_date_range` | listings | `ends_at IS NULL OR starts_at IS NULL OR ends_at >= starts_at` |
| `unique_save_per_user_listing` | interactions | `UNIQUE(user_id, listing_id, kind)` — prevents duplicate saves |
...
- [x] Production build passes (`npm run build` — Turbopack, 0 errors)
- [x] TypeScript type-checking passes
- [x] Static generation of `/` and `/_not-found` routes confirmed

---

## 7. Phase 2 — Pending Requirements

> [!IMPORTANT]
> None of the items below have been started. They represent the next implementation phase.
...
> [!NOTE]
> This document is the single source of truth for the DevDrift project state. Update it as phases are completed or requirements change. Other agents should read this file before making any changes to the codebase.
